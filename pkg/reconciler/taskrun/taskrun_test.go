...
var (
	...
	templatedTask = tb.Task("test-task-with-substitution", tb.TaskSpec(
		...
		tb.TaskResources(
			...
			tb.TaskResourcesOutput("missingimage", resourcev1alpha1.PipelineResourceTypeImage, tb.ResourceOptional(true)),
		),
		tb.Step("myimage", tb.StepName("mycontainer"), tb.StepCommand("/mycmd"), tb.StepArgs(
			...
			"--my-optional-arg=$(outputs.resources.missingimage.url)",
